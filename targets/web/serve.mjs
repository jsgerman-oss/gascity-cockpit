// Launch server for the web companion (the "launch script/command" deliverable
// of cockpit-dc8.7). Two jobs, zero dependencies:
//
//   1. Serve the static bundle in dist/web/.
...
// the API from one origin makes every `/v0` call same-origin, so Phase 0 runs
// end-to-end on localhost today without that server-side work. When the
// supervisor speaks CORS, the app can talk to it directly via `?endpoint=`.
//
// Localhost-only by design (binds 127.0.0.1): Phase 0 is the local-supervisor
...
const DEFAULTS = {
  ...
  dir: resolve(process.cwd(), "dist/web"),
};
...
// (RFC 2616 §13.5.1) — each side manages its own. Forwarding them is not just
// pedantically wrong: passing the browser's `Connection: keep-alive` through to
// the supervisor stalled larger `/v0` responses by ~2.5s here, so stripping them
// is also what makes the proxy fast. Node re-adds whatever it needs per hop.
...
function proxy(req, res, supervisor) {
  ...
  upstream.on("error", (err) => {
    ...
    res.end(`web companion: supervisor unreachable at ${supervisor} — ${err.message}\n`);
  });
...
async function serveStatic(req, res, dir) {
  ...
  try {
    ...
    try {
      ...
      res.writeHead(404, { "content-type": "text/plain; charset=utf-8" });
      res.end("web companion bundle not found — run `npm run build` first.\n");
      return;
    }
  }
...
function start(opts) {
  ...
  server.listen(opts.port, "127.0.0.1", () => {
    console.log(`GasCity Cockpit web companion`);
